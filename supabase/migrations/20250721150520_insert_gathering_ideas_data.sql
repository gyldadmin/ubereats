-- Migration: Insert gathering_ideas data from spreadsheet
-- Handles foreign key lookups and comma-delimited experience types

-- Create lookup CTEs for foreign key mappings
...
-- Raw data from spreadsheet with proper comma handling
raw_data AS (
  VALUES
...
processed_data AS (
  SELECT 
    ...
    rd.column5 as why,
    -- Split experience_type by comma and lookup UUIDs
    ARRAY(
...
SELECT 
  ...
  pd.category_ids,
  NULL as tag, -- Leave tag as NULL as requested
  NOW(),
  NOW()
...
BEGIN
  ...
  RAISE NOTICE 'Successfully inserted % gathering ideas with foreign key lookups', inserted_count;
...
DO $$
BEGIN
  RAISE NOTICE 'Verification - Total gathering_ideas count: %', (SELECT count(*) FROM gathering_ideas);
  RAISE NOTICE 'Sample data check - Ideas with multiple experience types: %', (
    SELECT count(*) FROM gathering_ideas 
    WHERE array_length(experience_type, 1) > 1
  );
END $$; 